...
export async function storeImages(data: storeImageInput[]) {
    ...
    for (const img of data) {
        ...
        const { data: dbData, error: dbError } = await supabase.from('generated-images').insert([{
            user_id: user.id,
            model: img.model,
            prompt: img.prompt,
...
export async function getImages(limit?: number) {
    ...
    let query = supabase.from("generated-images").select("*").eq("user_id", user.id).order("created_at", { ascending: false })
...
export async function deleteImage(id: string, imageName: string) {
    ...
    const { data, error } = await supabase.from('generated-images').delete().eq('id', id);
    if (error) {
        return {
